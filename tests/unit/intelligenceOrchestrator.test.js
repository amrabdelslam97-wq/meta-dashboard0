...
describe('intelligenceOrchestrator.runScoringPipeline optimization_goal threading', () => {
  ...
  test('a campaign-shaped entity (no optimization_goal) scores against the base objective profile', async () => {
    ...
    const { healthResult, benchmarkResult } = await runScoringPipeline(entity, metrics, null, 'acct-orch', 'campaign');
  ...
  test('an ad-set-shaped entity with a Video Views optimization_goal activates the sub-profile end to end', async () => {
    const entity = {
    ...
    const { healthResult, benchmarkResult } = await runScoringPipeline(entity, metrics, null, 'acct-orch', 'ad_set');
