...
describe('healthResolver.resolveHealthScore', () => {
  ...
  test('all engagement metrics present and excellent -> perfect score, identical to calculateHealthScore', async () => {
    ...
    const viaResolver = await resolveHealthScore(campaign, metrics, 'test-account-id');
    const viaWrapper  = await calculateHealthScore(campaign, metrics, 'test-account-id');
  ...
  test('no scoring config for an unrecognized objective returns neutral score with profile_key set', async () => {
    const campaign = { meta_campaign_id: 'camp_hr_2', name: 'Unknown Objective', objective: 'not_a_real_objective' };
    const result = await resolveHealthScore(campaign, { ctr: 5 }, 'test-account-id');
  ...
  test('profile_key reflects the base objective when no optimization_goal is passed', async () => {
    const campaign = { meta_campaign_id: 'camp_hr_3', name: 'Awareness Campaign', objective: 'awareness' };
    const result = await resolveHealthScore(campaign, { reach: 20000, cpm: 5, frequency: 2, impressions: 40000 }, 'test-account-id');
    expect(result.profile_key).toBe('awareness');
  });

  test('profile_key reflects the Video Views sub-profile when a matching optimization_goal is passed', async () => {
    ...
    const result = await resolveHealthScore(campaign, { reach: 20000, cpm: 5, frequency: 2, impressions: 40000 }, 'test-account-id', 'THRUPLAY');
    expect(result.profile_key).toBe('awareness.videoViews');
  });

  test('an ad-set optimization_goal that is not a Video Views goal does not activate the sub-profile', async () => {
    const campaign = { meta_campaign_id: 'camp_hr_5', name: 'Reach Awareness Campaign', objective: 'awareness' };
    const result = await resolveHealthScore(campaign, { reach: 20000, cpm: 5, frequency: 2, impressions: 40000 }, 'test-account-id', 'REACH');
    expect(result.profile_key).toBe('awareness');
  });
