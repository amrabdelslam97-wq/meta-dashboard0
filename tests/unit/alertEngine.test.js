...
describe('alertEngine.runAlertEngine objective_scope (T5-01)', () => {
  ...
  test('ROAS_BELOW_ONE fires for a sales campaign with roas < 1.0', async () => {
    const campaign = { meta_campaign_id: 'camp_alert_sales', name: 'Sales Campaign', objective: 'sales' };
    const alerts = await runAlertEngine(campaign, { roas: 0.5 }, null, 'acct-alert', 'campaign');
    expect(alerts.some(a => a.alert_code === 'ROAS_BELOW_ONE')).toBe(true);
  });

  test('ROAS_BELOW_ONE does NOT fire for a non-sales objective, even if a roas-shaped value is present', async () => {
  ...
  test('objective-unscoped rules (CPM_SPIKE, CTR_DROP) still fire for any objective', async () => {
    ...
    const alerts = await runAlertEngine(campaign, current, prior, 'acct-alert', 'campaign');
