...
// ── Phase 43 additions (Task 3: hook signal coverage; Task 4: psychology
// dimension coverage). Same convention as every list above -- small,
// reviewable, multi-word Arabic phrases preferred over single generic words
// to keep precision high. Never replaces an existing list/key; every
// pre-Phase-43 detected/missing key and psychology dimension keeps working
// exactly as before.
const OPEN_LOOP_WORDS = ['keep reading', 'wait until you see', 'here\'s what happened', 'by the end of this', 'you\'ll want to see this till the end'];
const OPEN_LOOP_WORDS_AR = ['لسه هتشوف', 'استني تشوف', 'خليك معايا لحد الاخر', 'هتفاجأ لما تعرف', 'كمل قراءه'];
const OPEN_LOOP_ALL = [...OPEN_LOOP_WORDS, ...OPEN_LOOP_WORDS_AR];

const STATISTIC_REGEX = /(\d+%|\d+٪|\d+\s*(out of|من)\s*\d+|\d+\/\d+)/i;
const SPECIFICITY_REGEX = /(\d+\s*(day|days|hour|hours|minute|minutes|week|weeks|\$))/i;
const SPECIFICITY_REGEX_AR = /(\d+\s*(يوم|ايام|ساعه|ساعات|دقيقه|دقايق|اسبوع|جنيه))/;

const SHOCK_WORDS = ['you won\'t believe', 'shocking', 'this is insane', 'wait till you see this'];
const SHOCK_WORDS_AR = ['مش هتصدق', 'صدمه', 'جامد اوي', 'هتتصدم'];
const SHOCK_ALL = [...SHOCK_WORDS, ...SHOCK_WORDS_AR];

const NOVELTY_WORDS = ['first of its kind', 'never seen before', 'brand new', 'just launched'];
const NOVELTY_WORDS_AR = ['لاول مره في مصر', 'جديد كليا', 'لسه نازل', 'اول مره تشوف كده'];
const NOVELTY_ALL = [...NOVELTY_WORDS, ...NOVELTY_WORDS_AR];

const PATTERN_INTERRUPT_WORDS = ['hold on a second', 'before you scroll away', 'wait for it', 'stop scrolling for a second'];
const PATTERN_INTERRUPT_WORDS_AR = ['لحظه واحده', 'استني شويه', 'قبل ما تكمل سكرول', 'خد بالك'];
const PATTERN_INTERRUPT_ALL = [...PATTERN_INTERRUPT_WORDS, ...PATTERN_INTERRUPT_WORDS_AR];

const FEAR_WORDS = ['don\'t let this happen to you', 'before it\'s too late'];
const FEAR_WORDS_AR = ['خايف يحصلك', 'لو اتاخرت هتندم', 'لا قدر الله'];
const FEAR_ALL = [...FEAR_WORDS, ...FEAR_WORDS_AR];

const LOSS_AVERSION_WORDS = ['don\'t miss out', 'you\'ll regret', 'missing out on'];
const LOSS_AVERSION_WORDS_AR = ['هتندم لو فوتها', 'متخسرش الفرصه دي', 'هتخسر لو استنيت'];
const LOSS_AVERSION_ALL = [...LOSS_AVERSION_WORDS, ...LOSS_AVERSION_WORDS_AR];

const IDENTITY_WORDS = ['for people who', 'if you\'re the type who'];
const IDENTITY_WORDS_AR = ['لناس زيك', 'لو انت من النوع اللي', 'دي لك لو'];
const IDENTITY_ALL = [...IDENTITY_WORDS, ...IDENTITY_WORDS_AR];

const BELONGING_WORDS = ['join our community', 'be part of'];
const BELONGING_WORDS_AR = ['انضم لعائلتنا', 'كن جزء من', 'من عائله'];
const BELONGING_ALL = [...BELONGING_WORDS, ...BELONGING_WORDS_AR];

const EXCLUSIVITY_WORDS = ['members only', 'exclusive access', 'invite only'];
const EXCLUSIVITY_WORDS_AR = ['حصريا لعملائنا', 'خاص بالمشتركين بس', 'دعوه خاصه'];
const EXCLUSIVITY_ALL = [...EXCLUSIVITY_WORDS, ...EXCLUSIVITY_WORDS_AR];

const STATUS_WORDS = ['be the first', 'stand out from the crowd'];
const STATUS_WORDS_AR = ['خليك مميز', 'كن الافضل', 'كن سباقا'];
const STATUS_ALL = [...STATUS_WORDS, ...STATUS_WORDS_AR];

const FUTURE_PACING_WORDS = ['imagine yourself', 'picture this', 'a month from now'];
const FUTURE_PACING_WORDS_AR = ['تخيل نفسك بعد شهر', 'تخيل حياتك وانت', 'بعد شهر من الان'];
const FUTURE_PACING_ALL = [...FUTURE_PACING_WORDS, ...FUTURE_PACING_WORDS_AR];

const RECIPROCITY_WORDS = ['free gift', 'on us', 'no strings attached'];
const RECIPROCITY_WORDS_AR = ['هديه منا', 'من غير مقابل', 'كوبون مجاني هديه'];
const RECIPROCITY_ALL = [...RECIPROCITY_WORDS, ...RECIPROCITY_WORDS_AR];

const COMMITMENT_WORDS = ['book your spot', 'reserve your place now'];
const COMMITMENT_WORDS_AR = ['احجز مكانك', 'خلي نفسك ملتزم', 'ابدا رحلتك معنا'];
const COMMITMENT_ALL = [...COMMITMENT_WORDS, ...COMMITMENT_WORDS_AR];
 ...
 * ؟), a hook-trigger word/phrase at the start (English or Arabic), direct
 * address ("you"/"انت", i.e. an audience call-out), a leading number, an
 * open loop, a statistic, specificity, shock, novelty, a pattern interrupt,
 * authority, transformation, pain, benefit, offer, urgency, and emotion.
 * Returns both a human-readable `evidence` string (backward compatible with
 * every existing caller/test) and structured `detected`/`missing` arrays
 * (Phase 41 explainability -- lets the frontend show a checklist instead of
 * a single opaque score).
 *
 * Phase 43 (Task 3): an emoji-led opening is tracked (`detected`/`missing`
 * still carry the `emoji` key, unchanged) but is now a small +5 bonus, not a
 * full persuasion signal -- "emoji alone is not a hook" was the explicit
 * finding: previously a creative with only an emoji and nothing else real
 * scored the same per-signal weight as a genuine question/curiosity/pain-
 * point hook. `PERSUASION_SIGNAL_KEYS` is exactly the set of real,
 * evidence-bearing categories the score is built from; emoji is excluded
 * from that count on purpose.
 */
const PERSUASION_SIGNAL_KEYS = new Set([
  'question', 'hook_trigger', 'direct_address', 'number', 'curiosity', 'surprise',
  'pain_point', 'benefit', 'offer', 'urgency', 'emotional_opening',
  'open_loop', 'statistic', 'specificity', 'shock', 'novelty', 'pattern_interrupt',
...
function analyzeHook(primaryText) {
  ...
  let hasEmoji = false;
  if (EMOJI_REGEX.test(opening)) { hasEmoji = true; detected.push({ key: 'emoji', label: 'Emoji-led opening' }); }
  ...
  if (DIRECT_ADDRESS_REGEX.test(openingHead) || DIRECT_ADDRESS_REGEX_AR.test(openingHead)) {
    detected.push({ key: 'direct_address', label: 'Audience call-out -- directly addresses the reader ("you"/"انت")' });
  } else {
    missing.push({ key: 'direct_address', label: 'Audience call-out (direct address)' });
  }
  ...
  // Phase 43 (Task 3) — additional hook dimensions.
  const openLoopHits = containsAny(opening, OPEN_LOOP_ALL);
  if (openLoopHits.length) detected.push({ key: 'open_loop', label: `Open loop -- promises information without giving it yet: "${openLoopHits[0]}"` });
  else missing.push({ key: 'open_loop', label: 'Open loop' });

  const statisticHit = STATISTIC_REGEX.test(opening);
  if (statisticHit) detected.push({ key: 'statistic', label: 'Cites a concrete statistic (%, ratio, or count)' });
  else missing.push({ key: 'statistic', label: 'Statistic' });

  const specificityHit = SPECIFICITY_REGEX.test(opening) || SPECIFICITY_REGEX_AR.test(opening) || statisticHit;
  if (specificityHit) detected.push({ key: 'specificity', label: 'Concrete/specific (number, timeframe, or price) rather than vague' });
  else missing.push({ key: 'specificity', label: 'Specificity' });

  const shockHits = containsAny(opening, SHOCK_ALL);
  if (shockHits.length) detected.push({ key: 'shock', label: `Shock signal: "${shockHits[0]}"` });
  else missing.push({ key: 'shock', label: 'Shock' });

  const noveltyHits = containsAny(opening, NOVELTY_ALL);
  if (noveltyHits.length) detected.push({ key: 'novelty', label: `Novelty signal: "${noveltyHits[0]}"` });
  else missing.push({ key: 'novelty', label: 'Novelty' });

  const patternInterruptHits = containsAny(opening, PATTERN_INTERRUPT_ALL);
  if (patternInterruptHits.length) detected.push({ key: 'pattern_interrupt', label: `Pattern interrupt: "${patternInterruptHits[0]}"` });
  else missing.push({ key: 'pattern_interrupt', label: 'Pattern interrupt' });

  const authorityHits = containsAny(opening, AUTHORITY_ALL);
  if (authorityHits.length) detected.push({ key: 'authority', label: `Authority signal: "${authorityHits[0]}"` });
  else missing.push({ key: 'authority', label: 'Authority' });

  const transformationHits = containsAny(opening, TRANSFORMATION_WORDS_AR);
  if (transformationHits.length) detected.push({ key: 'transformation', label: `Transformation framing: "${transformationHits[0]}"` });
  else missing.push({ key: 'transformation', label: 'Transformation framing' });
  ...
  // Each real PERSUASION signal category counts once toward the score,
  // regardless of how many keyword lists it matched under -- capped so a
  // handful of strong signals already reaches a strong score without
  // needing every single category to fire (2 solid signals + a well-sized
  // opening reaches "strong", matching this file's original 2-signals-is-
  // strong calibration). Emoji is deliberately NOT part of this count
  // (Phase 43, Task 3) -- it's a small +5 cosmetic bonus on top, so an
  // emoji-only opening with no real persuasion signal caps out weak/moderate,
  // never "strong" on emoji alone.
  const persuasionCount = detected.filter(d => PERSUASION_SIGNAL_KEYS.has(d.key)).length;
  const score = Math.min(100, persuasionCount * 20 + (lengthOk ? 20 : 0) + (hasEmoji ? 5 : 0));
 ...
 * reversal, and emotional appeal (English + Arabic). Phase 41 expanded this
 * from the original 5 dimensions to 13. Phase 43 (Task 4) adds fear, loss
 * aversion, identity, belonging, exclusivity, status, future pacing,
 * problem awareness, solution awareness, reciprocity, commitment, and trust
 * -- same "generic vocabulary list, never fabricated" approach as every
 * prior dimension, purely additive (no dimension removed/renamed, no schema
 * change -- this whole object is stored as JSON, not individual DB columns).
 *
 * `dimensions` (boolean map) and `evidence` (combined string) keep their
 * exact pre-Phase-43 shape/semantics for backward compatibility. `details`
 * is new: one entry per dimension explaining WHY it was detected (the real
 * matched phrase) or WHY it's absent (Task 4's explicit requirement).
 */
const PSYCHOLOGY_DIMENSION_LABELS = {
  urgency: 'Urgency', scarcity: 'Scarcity', curiosity: 'Curiosity', pain_point: 'Pain point',
  benefit: 'Benefit', emotional_appeal: 'Emotional appeal', transformation: 'Transformation',
  problem_solution: 'Problem/solution framing', social_proof: 'Social proof', authority: 'Authority',
  guarantee: 'Guarantee', value: 'Value', risk_reversal: 'Risk reversal', fear: 'Fear',
  loss_aversion: 'Loss aversion', identity: 'Identity', belonging: 'Belonging', exclusivity: 'Exclusivity',
  status: 'Status', future_pacing: 'Future pacing', problem_awareness: 'Problem awareness',
  solution_awareness: 'Solution awareness', reciprocity: 'Reciprocity', commitment: 'Commitment', trust: 'Trust',
};

function analyzePsychology(combinedText) {
  const hits = {
    urgency: containsAny(combinedText, URGENCY_ALL),
    scarcity: containsAny(combinedText, SCARCITY_WORDS_AR),
    curiosity: containsAny(combinedText, CURIOSITY_ALL),
    pain_point: containsAny(combinedText, PAIN_POINT_ALL),
    benefit: containsAny(combinedText, BENEFIT_ALL),
    emotional_appeal: containsAny(combinedText, EMOTION_ALL),
    transformation: containsAny(combinedText, TRANSFORMATION_WORDS_AR),
    problem_solution: containsAny(combinedText, PROBLEM_SOLUTION_WORDS_AR),
    social_proof: containsAny(combinedText, SOCIAL_PROOF_ALL),
    authority: containsAny(combinedText, AUTHORITY_ALL),
    guarantee: containsAny(combinedText, GUARANTEE_WORDS_AR),
    value: containsAny(combinedText, VALUE_WORDS_AR),
    risk_reversal: containsAny(combinedText, RISK_REVERSAL_WORDS_AR),
    // Phase 43 (Task 4) additions.
    fear: containsAny(combinedText, FEAR_ALL),
    loss_aversion: containsAny(combinedText, LOSS_AVERSION_ALL),
    identity: containsAny(combinedText, IDENTITY_ALL),
    belonging: containsAny(combinedText, BELONGING_ALL),
    exclusivity: containsAny(combinedText, EXCLUSIVITY_ALL),
    status: containsAny(combinedText, STATUS_ALL),
    future_pacing: containsAny(combinedText, FUTURE_PACING_ALL),
    // "Problem awareness" (the copy names a real pain/problem) reuses the
    // same pain-point vocabulary as the `pain_point` dimension above -- a
    // genuine conceptual overlap (naming a problem IS problem awareness),
    // not a second, independent detector. "Solution awareness" reuses the
    // existing solution-oriented phrase list for the same honest reason.
    problem_awareness: containsAny(combinedText, PAIN_POINT_ALL),
    solution_awareness: containsAny(combinedText, PROBLEM_SOLUTION_WORDS_AR),
    reciprocity: containsAny(combinedText, RECIPROCITY_ALL),
    commitment: containsAny(combinedText, COMMITMENT_ALL),
    // Trust reuses TRUST_ALL (the same list analyzeTrust() itself uses) so
    // the psychology composite doesn't silently disagree with the dedicated
    // Trust dimension elsewhere in this file.
    trust: containsAny(combinedText, TRUST_ALL),
  };

  const dimensions = Object.fromEntries(Object.entries(hits).map(([k, v]) => [k, v.length > 0]));
  const activeCount = Object.values(dimensions).filter(Boolean).length;
  // Scaled for the current dimension count -- a handful of real signals
  // still reaches a strong score without requiring every dimension to fire.
  const score = Math.min(100, activeCount * 15);

  const details = Object.entries(hits).map(([key, matchList]) => ({
    dimension: key,
    label: PSYCHOLOGY_DIMENSION_LABELS[key] || key,
    detected: matchList.length > 0,
    evidence: matchList.length > 0
      ? `Matched "${matchList[0]}".`
      : `No ${(PSYCHOLOGY_DIMENSION_LABELS[key] || key).toLowerCase()} language detected in the copy.`,
  }));

  const evidenceParts = details.filter(d => d.detected).map(d => `${d.label.toLowerCase()}: "${d.evidence.match(/"([^"]+)"/)[1]}"`);
