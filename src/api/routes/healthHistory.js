...
router.get('/', asyncHandler(async (req, res) => {
  const { entity_meta_id, entity_type = 'campaign', days = '30', since, until, account_id } = req.query;
  ...
  if (entity_meta_id) {
    ...
    const rows = db.all(
      ...
      account_id ? [entity_meta_id, entity_type, ...params, account_id] : [entity_meta_id, entity_type, ...params]
    );
