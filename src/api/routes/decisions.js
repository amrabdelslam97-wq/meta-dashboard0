...
router.get('/', asyncHandler(async (req, res) => {
  ...
  const result = await generateTodaysDecisions(account.id);

  // Persist to DB (idempotent — one per campaign+type per day)
  try { await persistDecisions(account.id, result.decisions); } catch {}
...
router.get('/winners', asyncHandler(async (req, res) => {
  ...
  const winners = await getTopWinners(limit, req.query.account_id || null);
...
router.get('/losers', asyncHandler(async (req, res) => {
  const limit = Math.min(parseInt(req.query.limit || '5', 10), 20);
  const losers = await getTopLosers(limit, req.query.account_id || null);
...
router.get('/opportunities', asyncHandler(async (req, res) => {
  ...
  const opps  = await detectAllOpportunities(limit, req.query.account_id || null);
...
router.get('/history', asyncHandler(async (req, res) => {
  ...
  let history = [];
  try {
    history = await getDecisionHistory(account.id, limit, status || null);
  } catch {
...
router.patch('/history/:id', asyncHandler(async (req, res) => {
  ...
  let row;
  try {
    row = await db.get('SELECT id FROM decision_history WHERE id = ?', [id]);
  } catch {
  ...
  if (action_taken !== undefined) {
    await db.run('UPDATE decision_history SET action_taken = ?, updated_at = ? WHERE id = ?',
      [action_taken ? 1 : 0, now, id]);
  }
  if (action_notes !== undefined) {
    await db.run('UPDATE decision_history SET action_notes = ?, updated_at = ? WHERE id = ?',
      [action_notes, now, id]);
  }
  if (status) {
    ...
    await db.run('UPDATE decision_history SET status = ?, updated_at = ? WHERE id = ?',
      [status, now, id]);
    if (status === 'completed') {
      await db.run('UPDATE decision_history SET completed_at = ?, action_taken = 1, updated_at = ? WHERE id = ?',
        [now, now, id]);
    }
    if (status === 'dismissed') {
      await db.run('UPDATE decision_history SET dismissed_at = ?, updated_at = ? WHERE id = ?',
        [now, now, id]);
    }
  }

  const updated = await db.get('SELECT * FROM decision_history WHERE id = ?', [id]);
